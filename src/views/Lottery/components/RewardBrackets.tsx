...
const RewardBrackets: React.FC<RewardMatchesProps> = ({ lotteryNodeData, isHistoricRound }) => {
  ...
  const getAzwRewards = (bracket: number) => {
      ...
      <RewardsInner>
        {rewardBrackets.map((bracketIndex) => (
          <RewardBracketDetail
            ...
            azwAmount={!isLoading && getAzwRewards(bracketIndex)}
            numberWinners={!isLoading && countWinnersPerBracket[bracketIndex]}
            isHistoricRound={isHistoricRound}
            isLoading={isLoading}
          />
        ))}
        <RewardBracketDetail rewardBracket={0} azwAmount={cakeToBurn} isBurn isLoading={isLoading} />
      </RewardsInner>
    </Wrapper>
  )
}
