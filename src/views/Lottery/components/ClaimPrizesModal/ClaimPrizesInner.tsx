...
import { AutoRenewIcon, Button, Flex, PresentWonIcon, Text } from '@pancakeswap/uikit'
...
const ClaimInnerContainer: React.FC<ClaimInnerProps> = ({ onSuccess, roundsToClaim }) => {
  ...
  const azwReward = activeClaimData.cakeTotal
  const dollarReward = azwReward.times(azwBusdPrice)
  const rewardAsBalance = getBalanceAmount(azwReward).toNumber()
    ...
    if (receipt?.status) {
      toastSuccess(
        ...
        <ToastDescriptionWithTx txHash={receipt.transactionHash}>
          {t('Your AZW prizes for round %lotteryId% have been sent to your wallet', { lotteryId })}
