...
const BuyTicketsModal: FC<BuyTicketsModalProps> = ({
  onDismiss,
  buyTicketCallBack,
  title,
  headerBackground,
  ...
  const isAzwBalanceInsufficient = azwBalance.lt(pricePerTicket)
  const maxBuyTickets = Math.min(azwBalance.div(pricePerTicket).toNumber(), remainingTickets)
        ...
        <Flex flexDirection="column">
          <Box px="16px">
            ...
            <Flex mb="8px" justifyContent="space-between">
              ...
              <Text font-size="14px">{formatBigNumber(pricePerTicket, 0)} AZW</Text>
            </Flex>
            <Flex mb="8px" justifyContent="space-between">
              <Text font-size="14px" color="textSubtle">
                {t('Your AZW Balance')}
              </Text>
              <Text font-size="14px" color={isAzwBalanceInsufficient ? 'failure' : 'text'}>
                {formatBigNumber(azwBalance, 3)} AZW
              </Text>
            </Flex>
            <Flex
              mb="8px"
            ...
            <Flex mb="25px" justifyContent="space-between">
              ...
              <Text font-size="14px" bold>
                {formatBigNumber(totalCost, 0)} AZW
              </Text>
            </Flex>
          </Box>
          <Flex
          ...
          <Box px="16px">
            <Button
              disabled={isAzwBalanceInsufficient}
              onClick={() => buyTicketCallBack({ ticketsNumber })}
              width="100%"
            >
              {isAzwBalanceInsufficient ? t('Insufficient Balance') : t('Confirm')}
            </Button>
          </Box>
        </Flex>
      </ModalBody>
