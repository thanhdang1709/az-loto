...
import {
  ...
  getCakePredictionsContract, getAzwContract,
...
export const useAzw = (): { reader: Cake; signer: Cake } => {
  const { account, library } = useActiveWeb3React()
  return useMemo(
      () => ({
        reader: getAzwContract(null),
        signer: getAzwContract(getProviderOrSigner(library, account)),
      }),
      [account, library],
  )
}
