...
const WalletInfo: FC<WalletInfoProps> = ({ hasLowBnbBalance, onDismiss }) => {
  const { t } = useTranslation()
  const { account } = useWeb3React()
  const { balance, fetchStatus } = useGetBnbBalance()
  const { balance: azwBalance, fetchStatus: cakeFetchStatus } = useTokenBalance(tokens.azw.address)
    ...
    <>
      ...
      <Flex alignItems="center" justifyContent="space-between">
        ...
        {fetchStatus !== FetchStatus.Fetched ? (
          ...
          <Text>{formatBigNumber(balance)}</Text>
        )}
      </Flex>
      <Flex alignItems="center" justifyContent="space-between" mb="24px">
        <Text color="textSubtle">{t('AZW Balance')}</Text>
        {cakeFetchStatus !== FetchStatus.Fetched ? (
          <Skeleton height="22px" width="60px" />
        ) : (
          <Text>{getFullDisplayBalance(azwBalance, 5, 5)}</Text>
        )}
      </Flex>
